...
const main = () => {
	...
	if ((template_slider && (page_name == 'gallery' || page_name == 'custom')) || template == 0) slider();
...
var template_slider = template == 0 || template == 7 || template == 8 || template == 9 || template == 11;

var nav_horizontal_right_left =
	template == 1 ||
	template == 4 ||
	template == 7 ||
	template == 11 ||
...
if (nav_horizontal_right_left) {
	...
	if (d.q('nav .buttons-wrap-header .btn')) {
		...
		btn_border = parseInt(getComputedStyle(d.q('nav .buttons-wrap-header .btn')).borderWidth);
